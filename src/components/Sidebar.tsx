...
const Sidebar: React.FC = () => {
  ...
  const APP_VERSION = 'v1.0.4'; // ★次回リリース時はここだけ更新
    ...
    <aside className="w-64 bg-slate-800 text-white flex flex-col flex-shrink-0 min-h-screen">
      <div className="h-16 flex items-center justify-center px-4 border-b border-slate-700">
        <div className="flex items-center gap-2">
          <LightbulbIcon />
          <div className="flex items-baseline gap-2 whitespace-nowrap overflow-visible">
            <h1 className="text-lg font-bold text-white">天才リーチ！</h1>
            <span className="text-[10px] text-slate-300 shrink-0">{APP_VERSION}</span>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 border-t border-slate-700 space-y-4">
        ...
        <p className="text-center text-[10px] text-slate-500 mt-1">VER: {APP_VERSION}</p>
      </div>
    </aside>
  );
};
